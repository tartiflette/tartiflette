...
class SchemaBakery:
    ...
    @staticmethod
    def _preheat(
        schema_name: str, parser: Optional[Callable] = None
    ) -> "GraphQLSchema":
        """
        ...
        sdl = schema_info["sdl"]
        schema = schema_from_sdl(sdl, schema_name=schema_name, parser=parser)
    ...
    async def bake(
        ...
        :rtype: GraphQLSchema
        """
        schema = SchemaBakery._preheat(schema_name, parser)
        await schema.bake(
