...
* arguments coercions are now fully working and directives on arguments directives are now properly coerced:
    ...
    type Query {
      hello(name: String!): @truncate(limit: 10)
    }
    ```
* on `on_pre_output_coercion` directive hook, the `parent` argument will be the complete coerced value instead of the coerced value of the field resolver only like on the previous versions:
    ```python
    """
    # SDL
    type Owner {
    ...
    async def resolve_query_dog(parent, args, ctx, info):
        return {"id": 1, "name": "Dog #1"}


    @Resolver("Dog.owner")
    async def resolve_query_dog(parent, args, ctx, info):
        return {"id": 1, "name": "Owner #1"}


    # Old
    @Directive("formatDog")
    class OldFormatDogDirective:
    ...
    class NewFormatDogDirective:
        async def on_pre_output_coercion(
            ...
            # `value` will be:
            # `{"id": 1, "name": "Dog #1", "owner": {"id": 1, "name": "Owner #1"}}`
            pass
    ```
...
* arguments coercion has been improved and we now make a strong distinction between `null` values and `undefined` values which could have an impact on your resolvers:
    ...
    async def resolve_mutation_add_news(parent, args, ctx, info):
      # `args` value will be:
      # Old versions: {"title": "Title", "resume": None, "content": "Content"}
      # New version: {"title": "Title", "content": "Content"}
