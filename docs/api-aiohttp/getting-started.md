...
* **engine_sdl**: Contains the [Schema Definition Language](https://graphql.org/learn/schema/)
  ...
  - Can be a path to a SDL
  - Can be an array of paths which contain the SDLs
* **engine_schema_name**: Name of the schema used by the built-in engine. Useful for advanced use-cases, see [API](/docs/api/engine).
...
* **graphiql_options** *(Optional[dict] = None)*: Customization options for the GraphiQL interface:
  ...
  - **default_headers** *(Optional[dict] = None)*: allows you to add default headers to each request sent through the GraphiQL interface