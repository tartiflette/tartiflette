...
* `schema_name` _(str = "default")_: name of the schema represented by the provided SDL ([more detail here](/docs/api/schema-registry/))
...
Both for your internal code and the plugins management, Tartiflette provides a `modules` parameter which give you the ability to specify all the internal and external modules you want to import. In addition to the module, you will be able to specify a configuration, which will be mostly used by the [Tartiflette plugin approach](/docs/plugin/introduction).
